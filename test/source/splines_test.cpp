...
TEST(SplinesRetardationTest, ConstantBcProducesFlatCurve) {
  constexpr float kBc = 0.300F;
  const std::array<float, 3> kMachs{0.5F, 1.0F, 2.0F};
  const std::array<float, 3> kBcs{kBc, kBc, kBc};
  std::array<float, lob::spline::kCoefsSize> coefs{};
  lob::spline::MakeRetardationCoefs(kMachs.data(), kBcs.data(), kMachs.size(),
                                    coefs.data());
  lob::spline::CurveView curve(lob::spline::kKnots, coefs);
  for (size_t i = 0; i < lob::spline::kKnotCount; ++i) {
    EXPECT_NEAR(curve.Eval(lob::spline::kKnots.at(i)), 1.0F / kBc, kEpsLoose)
        << "knot i=" << i;
  }
}

TEST(SplinesRetardationTest, ExtrapolatesFlatOutsideBandRange) {
  ...
  lob::spline::MakeRetardationCoefs(kMachs.data(), kBcs.data(), kMachs.size(),
                                    coefs.data());
  lob::spline::CurveView curve(lob::spline::kKnots, coefs);
  EXPECT_NEAR(curve.Eval(0.0F), 1.0F / kBcs[0], kEpsLoose);
  EXPECT_NEAR(curve.Eval(5.0F), 1.0F / kBcs[1], kEpsLoose);
}

TEST(SplinesRetardationTest, RuntimeMatchesCompileTimeVariant) {
  ...
  const auto kExpected =
      lob::spline::MakeRetardationCoefs<float, kN>(kMachs, kBcs);
  std::array<float, lob::spline::kCoefsSize> got{};
  lob::spline::MakeRetardationCoefs(kMachs.data(), kBcs.data(), kN, got.data());
...
TEST(SplinesMergeTest, DerivativesFollowProductRuleAtKnots) {
  ...
  for (size_t i = 0; i < lob::spline::kKnotCount; ++i) {
    ...
    EXPECT_NEAR(out.Deriv(kM), (kDya * b.Eval(kM)) + (a.Eval(kM) * kDyb),
                kEpsLoose)
        << "knot i=" << i;
  }
}
