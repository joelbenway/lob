...
namespace lob {
namespace boatright {
constexpr PsiT CalculateDynamicPressure(LbsPerCuFtT air_density,
                                        FpsT velocity) {
...
constexpr CaliberT CalculateRadiusOfTangentOgive(CaliberT ogive_length,
...
constexpr CaliberT CalculateFullNoseLength(CaliberT ogive_length,
                                           CaliberT meplat_diameter,
...
constexpr SqInT CalculateOgiveCrossSectionalArea(InchT x, InchT rho,
                                                 double alpha) {
...
constexpr double CalculateOgiveSimpsonIntegral(InchT a, InchT b, uint16_t n,
...
constexpr double CalculateOgiveVolume(InchT diameter, InchT ogive_length,
...
constexpr double CalculateFrustrumVolume(InchT d1, InchT d2, InchT length) {
...
constexpr double CalculateCylinderVolume(InchT diameter, InchT length) {
  return (std::pow(diameter / 2, 2) * kPi * length).Value();
}

constexpr double CalculateAverageDensity(
    InchT diameter, InchT length, InchT ogive_length, InchT ogive_full_length,
    InchT ogive_radius, InchT base_diameter, InchT tail_length, GrainT mass) {
...
constexpr double CalculateAverageDensity(InchT diameter, CaliberT length,
...
constexpr double CalculateFastAverageDensity(InchT diameter, InchT length,
...
constexpr double CalculateFastAverageDensity(InchT diameter, CaliberT length,
                                             CaliberT meplat_diameter,
                                             CaliberT ogive_length,
                                             CaliberT base_diameter,
...
constexpr double CalculateCoefficientOfLift(CaliberT full_ogive_length,
                                            MachT velocity) {
...
constexpr double CalculateCLBoattailAdjustmentFactor(PmsiT g7) {
  const double kBoattailAdjustmentFactor = std::sqrt(0.2720 / g7.Value());
  return kBoattailAdjustmentFactor;
}

constexpr double CalculateInertialRatio(InchT caliber, CaliberT length,
                                        CaliberT ogive_length,
...
constexpr HzT CalculateSpinRate(FpsT velocity, InchPerTwistT twist) {
  const double kInchesPerFoot = InchT(FeetT(1)).Value();
  return HzT(kInchesPerFoot * velocity.Value() / std::abs(twist.Value()));
}

constexpr double CalculateAspectRatio(CaliberT length,
                                      CaliberT full_ogive_length,
                                      CaliberT tail_length,
                                      CaliberT base_diameter) {
  const double kAR =
      length.Value() -
...
constexpr double CalculateYawDragCoefficient(MachT speed,
...
constexpr double CalculateEpicyclicRatio(double stability) {
  const double kSg = std::abs(stability);
  const double kR = (2 * (kSg + std::sqrt(kSg * (kSg - 1)))) - 1;
  return kR;
}

constexpr uint16_t CalculateNutationCyclesNeeded(double epicyclic_ratio) {
  const auto kN =
      static_cast<uint16_t>(std::floor((epicyclic_ratio - 1) / 4) + 1);
  return kN;
}

constexpr HzT CalculateGyroscopicRateSum(HzT spin_rate, double inertial_ratio) {
  return spin_rate / inertial_ratio;
}

constexpr HzT CalculateGyroscopicRateF2(HzT gyroscopic_rate_sum,
                                        double epicyclic_ratio) {
  return gyroscopic_rate_sum / (epicyclic_ratio + 1);
}

constexpr SecT CalculateFirstNutationPeriod(HzT f1, HzT f2) {
  assert(f1 > f2);
  const SecT kTn(1 / (f1.Value() - f2.Value()));
  return kTn;
}

constexpr double CalculateCrosswindAngleGamma(MphT zwind, FpsT velocity) {
  const double kGamma = FpsT(zwind).Value() / velocity.Value();
  return kGamma;
}

constexpr double CalculateZeroYawDragCoefficientOfDrag(double cd_ref,
                                                       GrainT mass,
                                                       InchT diameter,
                                                       PmsiT bc) {
...
constexpr double CalculateYawDragAdjustment(double gamma, double r,
                                            double cda) {
...
constexpr double CalculateVerticalPitch(double gamma, double r, double n) {
  const double kPitch = gamma * (((r * r) - 1) / (n * 2 * kPi * r)) *
                        (1 - std::cos(n * 2 * kPi / (r - 1)));
  return kPitch;
}

constexpr double CalculateVerticalImpulse(InchPerTwistT twist, uint16_t n,
                                          SecT tn, PsiT q, SqInT s, double cl,
                                          double cd, double pitch) {
...
constexpr double CalculateMagnitudeOfMomentum(GrainT mass, FpsT velocity) {
  const double kMOM =
...
constexpr double CalculateKOmega(InchT diameter, SecT supersonic_time) {
  const double kA = 0.585;
  const double kB = 0.0321;
  return -1 * (kA + kB / diameter.Value()) * supersonic_time.Value();
}

constexpr RadiansT CalculateYawOfRepose(FpsT initial_velocity,
                                        InchPerTwistT twist,
...
constexpr double CalculatePotentialDragForce(InchT diameter,
...
constexpr double CalculateCoefficientOfLiftAtT(double cl0,
                                               FpsT initial_velocity,
...
constexpr double CalculateSpinDriftScaleFactor(double potential_drag_force,
...
constexpr InchT CalculateSpinDrift(double scale_factor, InchT drop) {
...
constexpr double CalculateSpinDriftFactor(
    InchT diameter, InchT meplat_diameter, InchT base_diameter, InchT length,
    InchT ogive_length, InchT tail_length, double ogive_rtr, GrainT mass,
    FpsT velocity, double stability, InchPerTwistT twist,
    LbsPerCuFtT air_density, SecT supersonic_time) {
  ...
  const double kFormFactor = litz::CalculateG7FormFactorPrediction(
      diameter, kLN, kRTR, kDM, kLBT, kDB);
  const PmsiT kBcG7 =
      litz::CalculateBallisticCoefficient(mass, diameter, kFormFactor);
  const double kClBoattailAdjustment =
      boatright::CalculateCLBoattailAdjustmentFactor(kBcG7);
  const double kClOf0 =
      kClBoattailAdjustment *
      boatright::CalculateCoefficientOfLift(
